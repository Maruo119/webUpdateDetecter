...
data "aws_iam_policy_document" "lambda_permissions" {
  ...
  statement {
    ...
    resources = ["${aws_s3_bucket.state.arn}/cycleLifeBlog/*"]
  }
}
...
# ---------------------------------------------------------------------------
# Lambda function (package must be built with: pip install -r requirements.txt -t src/)
# ---------------------------------------------------------------------------
...
resource "aws_cloudwatch_event_rule" "hourly" {
  ...
  description         = "Trigger cycleLifeBlog update checker every hour"
  schedule_expression = var.schedule_expression
}

resource "aws_cloudwatch_event_target" "lambda" {
  rule      = aws_cloudwatch_event_rule.hourly.name
  target_id = "cycleLifeBlogChecker"
